...
void got_packet(u_char *handle, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    if (eth->ether_type == 0x0008)
    {
        ...
        // Parse UDP header
        if (iphdr->ip_proto == 0x11)
        {
            struct udp_header *udphdr = (udp_header *)(packet + ethhdr_size + iphdr_size);
            const int udphdr_size = 8;
            u_int sport = ((udphdr->sport & 0xff) << 8) + ((udphdr->sport >> 8) & 0xff);
            ...
            {
                printf("GRE Header detected...\n");
                char s_ip[20], cmd[200];
                ...
                const ethernet *in_eth = (ethernet *)(packet + ethhdr_size + iphdr_size + udphdr_size + grehdr_size);
