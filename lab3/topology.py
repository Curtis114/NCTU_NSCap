...
def config(routers, h2, h3, h4, DHCPServer):
    ...
    h3.cmd('python -m SimpleHTTPServer &')
    ...
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -o r1-eth0 -s 192.168.1.0/26 -j SNAT --to-source 140.113.0.30')
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -o r1-eth0 -s 192.168.1.64/26 -j SNAT --to-source 140.113.0.40')
    routers['r1'].cmd('iptables -t nat -A PREROUTING -i r1-eth0 -p tcp -d 140.113.0.40 --dport 80 -j DNAT --to 192.168.1.66:8000')
...
def runBGP(routers):
    routers['r1'].cmd('zebra -f ./configs/zebra.conf -d -i /var/run/quagga/zebra1.pid')
    routers['r1'].cmd('bgpd -f ./configs/bgp_r1.conf -d -i /var/run/quagga/bgpd1.pid')
    routers['r2'].cmd('zebra -f ./configs/zebra.conf -d -i /var/run/quagga/zebra2.pid')
    routers['r2'].cmd('bgpd -f ./configs/bgp_r2.conf -d -i /var/run/quagga/bgpd2.pid')
    routers['r3'].cmd('zebra -f ./configs/zebra.conf -d -i /var/run/quagga/zebra3.pid')
    routers['r3'].cmd('bgpd -f ./configs/bgp_r3.conf -d -i /var/run/quagga/bgpd3.pid')
    routers['r4'].cmd('zebra -f ./configs/zebra.conf -d -i /var/run/quagga/zebra4.pid')
    routers['r4'].cmd('bgpd -f ./configs/bgp_r4.conf -d -i /var/run/quagga/bgpd4.pid')

def killBGP():
    print("[-] Killing BGP")
    os.system("sudo kill -9 `cat /var/run/quagga/bgpd1.pid`")
    os.system("sudo kill -9 `cat /var/run/quagga/bgpd2.pid`")
    os.system("sudo kill -9 `cat /var/run/quagga/bgpd3.pid`")
    os.system("sudo kill -9 `cat /var/run/quagga/bgpd4.pid`")
    os.system("sudo kill -9 `cat /var/run/quagga/zebra1.pid`")
    os.system("sudo kill -9 `cat /var/run/quagga/zebra2.pid`")
    os.system("sudo kill -9 `cat /var/run/quagga/zebra3.pid`")
    os.system("sudo kill -9 `cat /var/run/quagga/zebra4.pid`")
